...
export async function changeUserName(data: User): Promise<Result> {
  try {
...
export async function changeUserPassword(data: PasswordFormData): Promise<Result> {
  try {
...
export async function changeUserMonthlySavingGoal(data: Setting): Promise<Result> {
  try {
    const res = await fetch(`/api/setting/saving-goal`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ monthlySavingGoal: data.monthlySavingGoal }),
      credentials: 'include'
    });
